...
contract('Test with fake balances and fake accounts', function(accounts) {
  ...
  it("should create the DGPS contract and have 500 DGPS tokens in contract", async function () {
    ...
    balanceBefore =  await web3.eth.getBalance(accounts[5]);
  })
  ...
  it('Adding fake accounts with 10k DGPT to the contract', async function() {
    let DGPS = await dgpsTokenContract.deployed();
    let DGPT = await dgptFake.deployed();

    let depositDGPT = await DGPS.addDGPSholder(accounts[5], 10000 * 1e18, {from: accounts[2]});
    assert.ok(depositDGPT);
  });
  ...
  it("Payout has been made automatically, profitbalance should be 0 again.", async function () {
    ...
    let userProfitsAfterUpdate = await DGPS.getHolderProfitBalance(accounts[5]);
    assert.equal(userProfitsAfterUpdate.valueOf(), 0, "1 Supply did not matched expected value");
